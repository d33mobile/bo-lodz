...
try { Object.assign(settings, JSON.parse(localStorage.getItem(SETKEY) || "{}")); } catch(e){}
function saveSettings(){ localStorage.setItem(SETKEY, JSON.stringify(settings)); }
let DATA = [];
let seen = new Set();
let favOrder = [];   // ordered list of favourite numery (source of truth)
let fav = new Set();
let shared = [], sharedSet = new Set();   // a shared list opened from #fav= (view only)
...
function render(){
  ...
  if(state.preset==="shared") items.sort((a,b)=> shared.indexOf(a.numer)-shared.indexOf(b.numer));
  ...
  const presetName = { all:"wszystkie", pon:"ogólnołódzkie", fav:"ulubione", shared:"udostępnione" }[state.preset];
  ...
  $("#count").innerHTML = esc(status)
    + (hiddenSeen > 0 ? ` · <a href="#" id="incSeen">ukryto ${hiddenSeen} odhaczonych — pokaż</a>` : "");
  $("#empty").hidden = items.length > 0;
  if(items.length === 0)
    $("#empty").textContent = hiddenSeen > 0
...
function exportCsv(){
  const cols = [["numer","Numer"],["tytul","Tytuł"],["typ","Typ"],["dzielnica","Dzielnica"],
                ...
                ["adres","Lokalizacja"],["opinia_rm","Opinia RM"],["opis","Opis"],["link","Link"]];
...
$("#list").addEventListener("click", e=>{
  ...
  if(o){
    if(ordHeld){ ordHeld = false; return; }   // long-press already moved it
    if(!o.disabled){
  ...
  if(f){
    ...
    if(adding && seen.has(n)){            // only if the setting marked it seen
...
$("#list").addEventListener("pointerdown", e=>{
  const o = e.target.closest(".ord"); if(!o || o.disabled) return;
  ordHeld = false;
  const up = "up" in o.dataset, n = up ? o.dataset.up : o.dataset.down;
  ordHoldTimer = setTimeout(()=>{ ordHeld = true; moveExtreme(n, !up); }, 450);
}, true);
function clearOrdHold(){ if(ordHoldTimer){ clearTimeout(ordHoldTimer); ordHoldTimer = null; } }
...
function toggleFav(n){
  ...
  if(adding){
    fav.add(n); favOrder.push(n);
    if(settings.favMarksSeen && !seen.has(n)){ seen.add(n); saveSeen(); }   // optional via settings
...
async function shareFav(){
  ...
  let hash;
  try {
    hash = ("CompressionStream" in window)
      ? "#favz=" + await gzipB64(favOrder.join(","))
      : "#fav=" + encodeURIComponent(favOrder.join(","));
  } catch(e){ hash = "#fav=" + encodeURIComponent(favOrder.join(",")); }
...
$("#negonly").addEventListener("change", e=>{ state.negonly=e.target.checked; render(); });
$("#count").addEventListener("click", e=>{
  if(e.target.id === "incSeen"){
    e.preventDefault();
    state.hideseen = false; $("#hideseen").checked = false; render();
  }
});
...
function ensureMap(){
  ...
  L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png",
    { maxZoom: 19, attribution: "© OpenStreetMap contributors",
      referrerPolicy: "no-referrer-when-downgrade" }).addTo(lmap);
...
function updateMap(){
  ...
  DATA.filter(p=>passes(p)).forEach(p=>{
    ...
    const m = L.circleMarker([p.lat, p.lon],
      { radius: settings.markerSize, weight: 2, color: col, fillColor: col, fillOpacity: .55 });
...
$("#setFavSeen").addEventListener("change", e=>{ settings.favMarksSeen = e.target.checked; saveSettings(); });
...
$("#wipe").addEventListener("click", ()=>{
  const w = $("#wipe");
  if(w.dataset.armed){                       // second click confirms
    Object.keys(localStorage).filter(k=>k.startsWith("bo-lodz-2026-2027-"))
      .forEach(k=>localStorage.removeItem(k));
    location.reload();
  } else {                                   // first click arms the confirm
    w.dataset.armed = "1"; w.classList.add("armed");
    w.textContent = "Na pewno? Kliknij ponownie, aby usunąć";
    setTimeout(()=>{ if(w.dataset.armed){
      delete w.dataset.armed; w.classList.remove("armed");
...
async function boot(d){
  ...
  [...new Set(DATA.map(p=>p.kategoria).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"pl"))
    .forEach(c=>{ const o=document.createElement("option"); o.value=o.textContent=c; $("#cat").appendChild(o); });
  $("#cat").value = state.cat;
  ...
  if(!boot.hashDone){   // a #favz=/#fav= link opens a view-only "Udostępnione" tab, once
    ...
    let listStr = null;
    if(mz){ try { listStr = await gunzipB64(mz[1]); } catch(e){ console.error(e); } }
    else if(m){ listStr = decodeURIComponent(m[1]); }
    if(listStr){
      const valid = new Set(DATA.map(p=>p.numer));
      shared = listStr.split(",").map(s=>s.trim()).filter(n=>valid.has(n));
...
(async function init(){
  try { localStorage.removeItem("bo-lodz-2026-2027-data-v1"); } catch(e){}
  let cached = null;
  ...
  try {
    ...
    try { localStorage.setItem(CACHE_KEY, JSON.stringify({ sig, data: fresh })); } catch(e){}
    if(sig !== shownSig){ shownSig = sig; await boot(fresh); }
  } catch(e){
    console.error(e);
    if(!cached){
